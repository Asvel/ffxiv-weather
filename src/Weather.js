...
let Weather = {
  ...
  calculateForecastTarget(timestamp) {
    ...
    let unix = Math.trunc(timestamp / 1000);
  ...
  init() {
    ...
    if (this._start === start) return;
    this._start = start;

    let futureET = Math.ceil(this.future * 24 * 60 * 60 / 175 / 8) + 1;
    let zones = Object.keys(this.zones);
    let weathers = {};
    for (let i = 0; i < futureET; i++) {
      let target = this.calculateForecastTarget(start + weatherDuration * i);
      for (let zone of zones) {
        let j = 0;
        while (target >= this.zoneRates[zone][j]) j++;
    ...
    this._get = (zone, i, j) => ({
      ...
      weathers: weathers[zone]
        ...
        .map(w => this.zones[zone][w]),
    });

    this._buffer = new Array(futureET * 8);
  },
  ...
  find(condition) {
    ...
    let desiredWeatherMask = condition.desiredWeatherMask ||
      this.zoneRates[zone].map((x, i) => hasWeather(condition.desiredWeathers, i));
    let previousWeatherMask = condition.previousWeatherMask ||
    ...
    for (let i = 8; i < matched.length; i++) {
      let weatherIndex = Math.floor(i / 8);
    ...
    while (true) {
      ...
      ret.push(this._get.bind(this, zone, i, j));
    }
    return ret;
  },
...
const hasWeather = (weathers, w) => !weathers || weathers.length === 0 || ~weathers.indexOf(w);
