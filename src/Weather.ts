...
export type Match = { begin: Date, end: Date, duration: number, weathers: string[] };
...
function init(): void {
  ...
  let futureET = Math.ceil(future * 24 * 60 * 60 / 175 / 8) + 1;
  let weathers = {} as typeof state.weathers;
  for (let i = 0; i < futureET; i++) {
    let target = calculateForecastTarget(start + weatherDuration * i);
  ...
  let buffer = new Array(futureET * 8);
...
export function find(condition: {
  ...
  let weathers = state.weathers[zone];

  let hourMask = condition.hourMask || Array.from({ length: 24 }, (x, i) => isHourIn(
    condition.beginHour || 0, condition.endHour == undefined ? 23 : condition.endHour, i));
  let baseHour = Math.round(state.start / 175 / 100);

  let matched = state.buffer.fill(false);
